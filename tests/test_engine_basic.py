...
def test_base_scenario_runs_without_error() -> None:
    ...
    result = run_all_scenarios(config, {"base_hit_everyone_happy": scenario_set.scenarios["base_hit_everyone_happy"]}, deals)[0]
...
def test_all_scenarios_produce_summary_rows() -> None:
    config, scenario_set, deals = load_inputs(ROOT / "inputs")
    results = run_all_scenarios(config, scenario_set.scenarios, deals)
