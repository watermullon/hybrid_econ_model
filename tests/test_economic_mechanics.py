...
from src.model_types import Scenario
...
def test_hf_returns_are_compounded_without_harvest_by_default() -> None:
    ...
    assert result.cashflows[0].hf_closing_nav == pytest.approx(1_000_000 * 1.20)
    assert result.cashflows[1].hf_opening_nav == pytest.approx(1_200_000)
    assert result.cashflows[1].hf_closing_nav == pytest.approx(1_200_000 * 1.15)
...
def test_hf_positive_return_harvest_moves_gain_to_retained_cash_when_not_distributed() -> None:
    ...
    assert year_one.hf_harvest == pytest.approx(100_000)
    assert year_one.hf_closing_nav == pytest.approx(1_100_000)
    assert year_one.retained_cash == pytest.approx(100_000)
